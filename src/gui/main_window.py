...
# ИСПРАВЛЕНИЕ для src/gui/main_window.py
# Заменить метод _configure_window в классе MainWindow:
    ...
    def _configure_window(self) -> None:
        ...
        # ИСПРАВЛЕНИЕ: Увеличенный размер окна для правильного отображения правой панели
        width = max(settings.gui_settings.window_width, 1200)  # Минимум 1200px ширины
        height = max(settings.gui_settings.window_height, 750)   # Минимум 750px высоты
        self.root.geometry(f"{width}x{height}")

        # ИСПРАВЛЕНИЕ: Увеличенный минимальный размер окна
        self.root.minsize(1000, 650)  # Было 800x600, стало 1000x650
